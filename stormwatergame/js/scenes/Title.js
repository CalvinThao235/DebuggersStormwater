...
var TitleState = {
  ...
  create: function () {
    ...
    // Register with TTSManager (handle load-order)
    const registerTTS = () => {
      ...
      // react to toggles (speak when enabled)
  ...
  playButtonActions: {
    onClick: function () {
      ...
      // cleanup tts handler
      if (window.TTSManager && this._ttsToggleHandler) window.TTSManager.off && window.TTSManager.off('tts-toggle', this._ttsToggleHandler);
